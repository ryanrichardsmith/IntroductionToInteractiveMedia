...
- There were a few difficulties I experienced:
  ...
  - Lastly, I tried my best to limit the configuration to a 3 x 4 table structure, but still have not figured out why the loop produces three infinite rows of fruits. In the end though, after moving the fruits closer together to add some layering, I felt like I had come full circle by adding a little bit of abstraction to an otherwise conventional piece. UPDATE: During class, I realized my conditional statements were inside the ''''draw()'''' function, which meant the fruits were just being drawn over and over every time there was a new frame. As a result, I moved the if statements to the ''''setup()'''' where I knew they would only run once.
