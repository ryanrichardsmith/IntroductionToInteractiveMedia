...
function setup() {
  ...
  //declaring the constant used to place the lines of the instructions at the same x coordinate
  const textXPos= 10
  ...
  text("You are a fisher looking for a good catch from the local lake.",textXPos,100);
  text("On each screen, some fish will randomly appear.", textXPos, 125)
  text("However, all of them are fake, except one.",textXPos,150)
  text("The real fish is a slightly different color so keep a sharp eye!",textXPos,175);
  text("Use the mouse to click on the real fish twice on its center.",textXPos,200)
...
//since I do not want the fish to be drawn multiple times a second, I decided to not use draw
function draw() {
}
...
class drawFish {
  ...
  draw() {
    fill(this.fill);
    triangle(this.x-20, this.y, this.x-75, this.y+20, this.x-75,this.y-20);
...
function mouseClicked() {
  ...
  state++;
    ...
    } else if (state === 1 && dist(uniqueFishX,uniqueFishY,mouseX,mouseY) < 50) {
